...
export async function cbk_fulFill_data_order(product: ProductTypes, order: OrderType) {
  ...
  const url = `${await initializeCBK({
    route: 'APIBuy.asp',
  })}&MobileNetwork=${companyData.ID}&DataPlan=${productData.PRODUCT_ID}&MobileNumber=${
    order.customerId
  }`;
